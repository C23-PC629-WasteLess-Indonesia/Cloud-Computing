    ...
    User.findOne({ where: { email } })
      .then((user) => {
        if (!user) {
          return res
            .status(401)
            .json({ error: "Invalid email/password. Please try again." });
        }
...
app.post(
  "/postFood",
  authenticateToken,
  upload.single("fotoMakanan"),
  async (req, res, next) => {
    try {
      const { foodName, description, quantity, location, expiredAt, foodType } =
        req.body;
      ...
      const response = await axios.get(
        `https://maps.googleapis.com/maps/api/geocode/json?address=${location}&key=${gmapsApiKey}`
      );
              ...
              User.findByPk(userId)
                .then((user) => {
                  if (user) {
                    ...
                    const newHistory = `${currentHistory}\n${foodName} - ${moment().format(
                      "DD-MM-YYYY"
                    )}`; // Format tanggal saat ini menjadi dd-mm-yyyy
                    user
                      .update({ historyDonation: newHistory })
                      .then(() => {
                        res
                          .status(200)
                          .json({
    ...
    .then((food) => {
      if (food.length === 0) {
        res
          .status(200)
          .json({
            message: "Tidak ada makanan yang didonasikan oleh pengguna ini",
            data: [],
          });
      } else {
...
app.get("/preferancefoodList", authenticateToken, async (req, res) => {
  try {
    ...
    const userId = get_user_id_from_auth_token(
            ...
            .then((food) => {
              if (food.length === 0) {
                res
                  ...
                  .json({
                    message:
                      "Tidak ada makanan yang direkomendasikan untuk saat ini",
                  });
              } else {
    ...
    Food.findAll()
      .then((food) => {
        if (food.length === 0) {
          res
            .status(200)
            .json({ message: "Tidak ada makanan yang sedang didonasikan" });
        } else {
          res.json(food);
        }
      })
      ...
      if (file) {
        ...
        blobStream.on("finish", async () => {
          try {
            ...
            User.update(
              { location, fotoProfile: publicUrl },
              { where: { userId } }
            )
              .then(() => {
                res
